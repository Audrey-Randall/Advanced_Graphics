...
//
//  Constructor: Ex01opengl(QWidget* parent) is a member function of class Ex01opengl, that's what the :: means.
// The single ':' means "initialize the constructor for QGLWidget first, before executing constructor body" since Ex01opengl inherits from QGLWidget
//
...
void Ex01opengl::listShaders(){
    shaderLocs.push_back(":/ex01");
    shaderLocs.push_back(":/perlin");
    shaders.push_back(new QGLShaderProgram);
    shaders.push_back(new QGLShaderProgram);
}
...
void Ex01opengl::initializeGL()
{
   ...
   //  Build shader
   for(int i = 0; i < shaderLocs.size(); i++) {
       //if (!shader.addShaderFromSourceFile(QGLShader::Vertex,":/ex01.vert"))
       if (!shaders[i]->addShaderFromSourceFile(QGLShader::Vertex,shaderLocs[i]+".vert"))
          Fatal("Error compiling vertex shader\n"+shaders[i]->log());
       if (!shaders[i]->addShaderFromSourceFile(QGLShader::Fragment,shaderLocs[i]+".frag"))
          Fatal("Error compiling fragment shader\n"+shaders[i]->log());
       if (!shaders[i]->link())
